...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`p-5 rounded-2xl transition-all duration-150 relative overflow-hidden group ${
      ...
      <div className="flex items-center justify-between">
        <span
          className={`text-xs font-semibold uppercase tracking-wider ${
            isOrange ? 'text-white/80' : isColored ? 'text-slate-700' : 'text-slate-500 dark:text-slate-400'
          }`}
        >
          {title}
        </span>
        <div
          className={`p-2.5 rounded-xl ${
            isOrange
              ...
              : 'bg-slate-100 dark:bg-slate-800 text-[var(--color-secondary)]'
          }`}
        >
          {icon}
        </div>
      ...
      <div className="mt-4 flex items-baseline justify-between">
        <span
          className={`text-3xl font-extrabold tracking-tight ${
            isOrange ? 'text-white' : 'text-slate-900 dark:text-white'
          }`}
        >
          {value}
        </span>
      ...
      {subtitle && (
        <p
          className={`text-xs mt-1.5 ${
            isOrange ? 'text-white/75' : isColored ? 'text-slate-600' : 'text-slate-400 dark:text-slate-500'
          }`}
        >
          {subtitle}
        </p>
