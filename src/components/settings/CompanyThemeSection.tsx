...
export const CompanyThemeSection: React.FC<CompanyThemeSectionProps> = ({ companyId, canModify }) => {
  const { data: dbTheme } = useCompanyTheme(companyId);
  ...
  useEffect(() => {
    if (dbTheme) {
      setFormState({
        ...
        primary_hover: dbTheme.primary_hover || DEFAULT_THEME_FIELDS.primary_hover,
        ...
        accent_soft: dbTheme.accent_soft || DEFAULT_THEME_FIELDS.accent_soft,
        ...
        dark_neutral: dbTheme.dark_neutral || DEFAULT_THEME_FIELDS.dark_neutral,
        text_color: dbTheme.text_color || dbTheme.color_5 || DEFAULT_THEME_FIELDS.text_color,
        border_color: dbTheme.border_color || DEFAULT_THEME_FIELDS.border_color,
      });
      setHasChanges(false);
  ...
  const handleSave = () => {
    ...
    if (invalidKey) {
      setValidationError(`Invalid hex code for ${invalidKey[0].replace(/_/g, ' ')}: "${invalidKey[1]}". Format must be 6-digit hex (e.g. #C1121F).`);
      return;
    }
  ...
  const darkNeutralVsText = getContrastRatio(formState.dark_neutral, '#FFFFFF');
  const isLowContrast = (bgVsText < 4.5 && surfaceVsText < 4.5) || primaryVsText < 3.0 || darkNeutralVsText < 3.0;
  ...
  const colorFields: { key: keyof typeof DEFAULT_THEME_FIELDS; label: string }[] = [
    ...
    { key: 'dark_neutral', label: 'Dark Neutral / Sidebar' },
      ...
      <div className="flex items-center justify-between pb-3 border-b border-[var(--color-border)]">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-stone-400">Customize the visual theme for your company.</p>
          </div>
        </div>
      ...
      <div className="space-y-2 pt-2 border-t border-[var(--color-border)]">
        ...
        <div
          ...
          style={{
            ...
            color: formState.text_color,
            borderColor: formState.border_color,
          }}
        >
          {/* Header */}
          <div
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div
              ...
              style={{ backgroundColor: formState.dark_neutral, color: '#FFFFFF' }}
            >
              <div className="font-bold text-[10px] opacity-75 uppercase">Sidebar</div>
              <div className="p-1 rounded bg-white/10 font-semibold">Dashboard</div>
              <div className="p-1 rounded opacity-70">Engineers</div>
              <div className="p-1 rounded opacity-70">Schedule</div>
            </div>

            {/* Card & Button Preview */}
            <div
              className="col-span-3 p-3 rounded-lg border shadow-2xs space-y-2"
              style={{
                backgroundColor: formState.surface_color,
                borderColor: formState.border_color,
                color: formState.text_color,
              }}
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  ...
                  style={{ backgroundColor: formState.accent_soft, color: formState.accent_color }}
                >
                  Active Accent
                </span>
              </div>
              <p className="text-[11px] opacity-80 leading-relaxed" style={{ color: formState.text_color }}>
                Engineer Deployment & Operations Overview
              </p>
